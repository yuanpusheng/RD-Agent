.PHONY: clean deepclean install init-qlib-env dev constraints black isort mypy ruff toml-sort lint pre-commit test-run test build upload docs-autobuild changelog docs-gen docs-mypy docs-coverage docs monitor-lint monitor-test monitor-image monitor-compose
...
########################################################################################
# Monitoring helper targets
########################################################################################

monitor-lint:
    $(PIPRUN) ruff check $(MONITOR_SOURCES) --ignore FBT001,FBT002,I001

monitor-test:
    $(PIPRUN) python -m pytest $(MONITOR_TESTS)

monitor-image:
    docker build -f deploy/monitoring/Dockerfile -t rdagent-monitor:latest .
